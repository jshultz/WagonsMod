...
public class EntityCoveredWagon extends Entity
{
	protected NBTTagCompound extPropCompound = new NBTTagCompound();

	/** true if no player in wagon */
	private boolean isWagonEmpty;
	private double speedMultiplier;
	private int wagonPosRotationIncrements;
	private double wagonX;
	private double wagonY;
	private double wagonZ;
	private double wagonYaw;
	...
	public EntityCoveredWagon(World world, double x, double y, double z)
	{
		this(world);
	...
	public boolean attackEntityFrom(DamageSource damageSource, float damageMultiplier)
	{
		...
		} else if (!this.worldObj.isRemote && !this.isDead)
		{
			...
			this.setDamageTaken(this.getDamageTaken() + damageMultiplier * 10.0F);
	...
	/**
	 * This method gets called when EntityInteractEvent is triggered. Set animal
	 * entity(s) to be anchored to front of wagon. Eventally to be moved to
	 * EntityWagon class to handle any type of wagon.
	 * 
	 * @param entity
	 */
	public void anchorAnimalToWagon(EntityPlayer player, World currentWorld, int posX,
			int posY, int posZ)
	{
		double minMaxAABB = 7.0D;

		// Get list of all entities leashed by player
		List entityList = currentWorld.getEntitiesWithinAABB(
				EntityLiving.class,
				AxisAlignedBB.getBoundingBox((double) posX - minMaxAABB, (double) posY
						- minMaxAABB, (double) posZ - minMaxAABB, (double) posX
						- minMaxAABB, (double) posY + minMaxAABB, (double) posZ
						+ minMaxAABB));
		...
		if (entityList != null && entityList.size() <= 2)
		{
			...
			while (iter.hasNext())
			{
				...
				{
					// DEBUG:
					System.out.println("[Steve's Wagons] successfully anchored "
							+ entity.toString() + " to wagon.");
				}
			}
		}
	}
	 ...
	 */
	@SideOnly(Side.CLIENT)
	public void setPositionAndRotation2(double posX, double posY, double posZ, float yaw,
			float pitch, int p_70056_9_)
	{
		if (this.isWagonEmpty)
		{
			this.wagonPosRotationIncrements = p_70056_9_ + 5;
		} else
		{
			...
			this.wagonPosRotationIncrements = 3;
		}
	...
	public void onUpdate()
	{
		...
		for (int i = 0; i < b0; ++i)
		{
			double d1 = this.boundingBox.minY
					+ (this.boundingBox.maxY - this.boundingBox.minY) * (double) (i + 0)
					/ (double) b0 - 0.125D;
			double d3 = this.boundingBox.minY
					+ (this.boundingBox.maxY - this.boundingBox.minY) * (double) (i + 1)
					/ (double) b0 - 0.125D;
			AxisAlignedBB axisalignedbb = AxisAlignedBB.getBoundingBox(
					this.boundingBox.minX, d1, this.boundingBox.minZ,
		...
		double distLast = Math.sqrt(this.motionX * this.motionX + this.motionZ
				* this.motionZ);
		double d2;
		double d4;
		int j;

		if (distLast > 0.26249999999999996D)
		{
			d2 = Math.cos((double) this.rotationYaw * Math.PI / 180.0D);
			d4 = Math.sin((double) this.rotationYaw * Math.PI / 180.0D);

			for (j = 0; (double) j < 1.0D + distLast * 60.0D; ++j)
			{
				...
				if (this.rand.nextBoolean())
				{
					...
					this.worldObj.spawnParticle("splash", d8, this.posY - 0.125D, d9,
		...
		if (this.worldObj.isRemote && this.isWagonEmpty)
		{
			if (this.wagonPosRotationIncrements > 0)
			{
				d2 = this.posX + (this.wagonX - this.posX)
						/ (double) this.wagonPosRotationIncrements;
				d4 = this.posY + (this.wagonY - this.posY)
						/ (double) this.wagonPosRotationIncrements;
				d11 = this.posZ + (this.wagonZ - this.posZ)
						/ (double) this.wagonPosRotationIncrements;
				d12 = MathHelper.wrapAngleTo180_double(this.wagonYaw
						- (double) this.rotationYaw);
				this.rotationYaw = (float) ((double) this.rotationYaw + d12
						/ (double) this.wagonPosRotationIncrements);
				this.rotationPitch = (float) ((double) this.rotationPitch + (this.wagonPitch - (double) this.rotationPitch)
						/ (double) this.wagonPosRotationIncrements);
				--this.wagonPosRotationIncrements;
		...
		} else
		{
					...
					&& this.riddenByEntity instanceof EntityLivingBase)
			{
				...
				this.motionX += -Math.sin((double) (f * (float) Math.PI / 180.0F))
						* this.speedMultiplier * (double) entitylivingbase.moveForward
			...
			if (d2 > distLast && this.speedMultiplier < 0.35D)
			{
			...
			for (l = 0; l < 4; ++l)
			{
				int i1 = MathHelper.floor_double(this.posX + ((double) (l % 2) - 0.5D)
						* 0.8D);
			...
			if (this.isCollidedHorizontally && distLast > 0.2D)
			{
	...
	public boolean interactFirst(EntityPlayer playerEntity)
	{
		if (this.riddenByEntity != null && this.riddenByEntity instanceof EntityPlayer
				&& this.riddenByEntity != playerEntity)
		{
			return true;
		} else
		{
